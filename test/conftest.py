...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # configured path does not exist -> treat as skippable
            return False
    else:
    ...
    if not exe_found:
        # executable not found in PATH -> treat as skippable rather than failing
        if old_environ is not None:
            os.environ.clear()
            os.environ.update(old_environ)
        return False
    elif tool_path is not None and tool_path not in exe_found:
        # finds the exe in a path that is not the one set in the conf -> treat as skippable
        if old_environ is not None:
            os.environ.clear()
            os.environ.update(old_environ)
        return False
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        if result is True:
            ...
            # Treat missing external tools as skippable instead of failing the whole test run
            pytest.skip(
