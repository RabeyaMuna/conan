...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # configured path does not exist -> treat as missing/skip (not hard error)
            return False
    else:
        if version is not None:  # if the version is specified, it should be in the conf
            # requested specific version not present in configuration -> skip
            return False
        tool_path = None
    ...
    if not exe_found:
        # exe not found -> treat as missing/skip rather than hard failure
        cached = False
        if tool_path is None:
            # will skip the test, not exe found and path None
            cached = False
    elif tool_path is not None and tool_path not in exe_found:
        # finds the exe in a path that is not the one set in the conf -> treat as missing/skip
        cached = False
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        if result is True:
            # Treat missing configured tool as skip rather than hard failure
