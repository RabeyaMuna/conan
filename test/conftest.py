...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # Configured path does not exist -> treat as a skip instead of a hard failure
            return False
    else:
    ...
    if not exe_found:
        # If the executable is not found and no explicit tool_path was configured, treat it as a skippable
        # external dependency rather than a hard failure.
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        if result is True:
            ...
            # Treat unavailable tools as a skip instead of a hard failure to avoid aborting the whole test run
            pytest.skip(
