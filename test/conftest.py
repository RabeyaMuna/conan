...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # Configured path doesn't exist: don't fail immediately, fall back to PATH lookup
            configured_but_missing = True
            tool_path = None
    else:
        if version is not None:  # if the version is specified, it should be in the conf
            # Don't fail immediately if a specific version is requested but not configured.
            # Allow falling back to finding the executable on the system PATH.
            pass
        tool_path = None
    ...
    if not exe_found:
        # exe not found on PATH (or in the configured path)
        if configured_but_missing or tool_path is None:
            # will fail the test: not exe found and no usable configured path
            cached = True
        else:
            cached = True
