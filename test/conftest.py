...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # Path configured but doesn't exist -> skip tests
            return False
    else:
        if version is not None:  # if the version is specified, it should be in the conf
            # Requested version not configured -> skip tests
            return False
        tool_path = None
    ...
    if not exe_found:
        # Executable not found -> skip tests
        cached = False
        if tool_path is None:
            # Not configured and executable missing -> skip tests
            cached = False
    elif tool_path is not None and tool_path not in exe_found:
        # finds the exe in a path that is not the one set in the conf -> skip tests
        cached = False
