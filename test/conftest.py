...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # Configured path does not exist: treat as missing tool and skip tests rather than error
            return False
    else:
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        if result is True:
            ...
            # Treat missing/invalid tools as skipped tests instead of failing the whole run
            pytest.skip(
