...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # configured path does not exist: fall back to probing PATH instead of failing
            tool_path = None
    else:
    ...
    if not exe_found:
        # do not hard-fail tests; indicate missing tool so pytest can skip
        cached = False
    elif tool_path is not None and tool_path not in exe_found:
        # finds the exe in a path that is not the one set in the conf -> use found exe instead of failing
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        # Do not hard-fail tests for missing external tools. Treat both True and False as skips.
