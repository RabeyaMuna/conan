...
def _get_individual_tool(name, version):
    ...
    if tool_version is not None:
        ...
        if tool_path == "skip-tests":
            ...
            # configured directory doesn't exist: do not fail, try to discover executable in PATH
            tool_path = None
    else:
        if version is not None:  # if the version is specified, it should be in the conf
            # Requested version not present in configuration: skip tests instead of failing
            return False
        tool_path = None
    ...
    if not exe_found:
        # Executable not found on PATH: skip tests
        if old_environ is not None:
            os.environ.clear()
            os.environ.update(old_environ)
        return False

    # Normalize paths and accept the executable found even if it's not exactly the configured path
    if tool_path is not None:
        try:
            norm_tool = os.path.normpath(os.path.realpath(tool_path))
            norm_exe_dir = os.path.normpath(os.path.realpath(exe_path))
        except Exception:
            norm_tool = tool_path
            norm_exe_dir = exe_path
        # Do not treat a different location as a fatal error; accept the found executable
...
def pytest_runtest_setup(item):
    ...
    for tool_params in tools_params:
        ...
        # _get_tool/_get_individual_tool should return:
        #  - (path, env) tuple when available
        #  - False when the tool is disabled or not found (skip tests)
        #  - (None, None) for platform skips
        if result is True:
            # Historical behavior treated True as fatal; prefer skipping tests instead
