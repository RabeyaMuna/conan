...
class TestMesonBase(unittest.TestCase):
    ...
    def _check_binary(self):
        ...
        if platform.system() == "Darwin":
            ...
            self.assertIn(f"main {arch_macro['gcc'][host_arch]} defined", self.t.out)
            # Check for GCC major macro without hardcoding a specific version
            import re

            m = re.search(r"main __GNUC__\d+", self.t.out)
            self.assertIsNotNone(
                m,
                f"Expected GCC major macro (e.g. '__GNUC__<N>') in output, got: {self.t.out}",
            )
