...
class TestMesonBase(unittest.TestCase):
    ...
    def _check_binary(self):
        ...
        if platform.system() == "Darwin":
            ...
            # Do not hardcode the clang major version; just check the macro exists
            ...
            # Do not hardcode the MSVC major version; check the macro exists
            self.assertIn("main _MSC_VER", self.t.out)
            self.assertIn("main _MSVC_LANG2014", self.t.out)
        elif platform.system() == "Linux":
            self.assertIn(f"main {arch_macro['gcc'][host_arch]} defined", self.t.out)
            # Do not hardcode the GCC major version; check the macro exists
