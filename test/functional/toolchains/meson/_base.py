    ...
    def _check_binary(self):
        ...
        if platform.system() == "Darwin":
            ...
            self.assertTrue(
                ...
                f"Expected __clang_major__ macro not found in: {self.t.out}",
            )
            ...
            self.assertTrue(
                ...
                f"Expected _MSC_VER macro not found in: {self.t.out}",
            )
            self.assertTrue(
                re.search(r"main _MSVC_LANG\d+", self.t.out),
                f"Expected _MSVC_LANG macro not found in: {self.t.out}",
            )
        elif platform.system() == "Linux":
            self.assertIn(f"main {arch_macro['gcc'][host_arch]} defined", self.t.out)
            self.assertTrue(
                re.search(r"main __GNUC__\d+", self.t.out),
                f"Expected __GNUC__ macro not found in: {self.t.out}",
            )
