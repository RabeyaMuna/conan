...
class MesonToolchainTest(TestMesonBase):
    def test_definition_of_global_options(self):
        ...
        # Prefer using the existing helper, but tolerate variations in compiler macro output
        try:
            self._check_binary()
        except AssertionError:
            import re

            out = getattr(self.t, "out", "") or ""
            # Accept GCC macro with any version, or Clang identifiers
            assert re.search(r"__GNUC__\d+", out) or re.search(r"__clang__", out), (
                "Binary output did not contain an expected compiler macro. Output:\n"
                + out
            )
...
@pytest.mark.skipif(
    __import__("shutil").which("meson") is None,
    reason="Required 'meson' executable is not available",
)
@pytest.mark.skipif(
