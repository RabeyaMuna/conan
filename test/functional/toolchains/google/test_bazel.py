...
@pytest.mark.skipif(
    ...
    reason="Requires 'bazel' tool (version 6.5.0). Skipping if not available in CI.",
)
...
@pytest.mark.skipif(
    shutil.which("bazel") is None,
    reason="Requires 'bazel' tool (version 7.4.1). Skipping if not available in CI.",
)
...
@pytest.mark.skipif(
    shutil.which("bazel") is None,
    reason="Requires 'bazel' tool (version 8.0.0). Skipping if not available in CI.",
)
