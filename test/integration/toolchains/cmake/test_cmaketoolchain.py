...
def test_cross_build_user_toolchain():
    ...
    client.save(
        {"conanfile.py": conanfile, "rpi": rpi_profile, "windows": windows_profile}
    )
    client.run("install . --profile:build=windows --profile:host=rpi")
    toolchain = client.load("conan_toolchain.cmake")
...
def test_no_cross_build():
    ...
    client.save({"conanfile.py": conanfile, "windows": windows_profile})
...
def test_no_cross_build_arch():
    ...
    client.save(
        {"conanfile.py": conanfile, "linux64": build_profile, "linux32": profile_32}
    )
...
def test_runtime_lib_dirs_single_conf(lib_dir_setup):
    ...
    lib_path = (
        cmake_library_path.replace("/p/lib", "/p/bin")
        if is_windows
...
def test_apple_vars_overwrite_user_conf():
    ...
    client.save({"profile_ios": profile_ios})
    client.run(
        "install hello.py -pr:h=./profile_ios -pr:b=default -g CMakeToolchain "
...
def test_cmake_presets_shared_preset(presets):
    ...
    client.save(
        {presets: project_presets, "conanfile.py": conanfile, "CMakeLists.txt": ""}
    )  # File must exist for Conan to do Preset things.
...
def test_cmake_presets_multiconfig():
    ...
    client.run(
        "install --requires=mylib/1.0@ -g CMakeToolchain "
...
def test_toolchain_cache_variables():
    ...
    with mock.patch("platform.system", mock.MagicMock(return_value="Windows")):
        client.run(
            "install . --name=mylib --version=1.0 "
    ...
    client.run(
        "install . --name=mylib --version=1.0 -c tools.gnu:make_program='MyMake'"
    )
    presets = json.loads(client.load("CMakePresets.json"))
    cache_variables = presets["configurePresets"][0]["cacheVariables"]
    assert cache_variables["CMAKE_MAKE_PROGRAM"] == "MyMake"
...
def test_android_legacy_toolchain_flag(cmake_legacy_toolchain):
    client = TestClient()
    conanfile = (
        GenConanfile().with_settings("os", "arch").with_generator("CMakeToolchain")
    )
    client.save({"conanfile.py": conanfile})
    settings = (
        "-s arch=x86_64 -s os=Android -s os.api_level=23 -c tools.android:ndk_path=/foo"
...
@pytest.mark.parametrize(
    "arch, arch_toolset",
    [("x86", "x86_64"), ("x86_64", "x86_64"), ("x86", "x86"), ("x86_64", "x86")],
)
def test_presets_ninja_msvc(arch, arch_toolset):
    ...
    client.save({"conanfile.py": conanfile, "CMakeLists.txt": "foo"})
    configs = [
        "-c tools.cmake.cmaketoolchain:toolset_arch={}".format(arch_toolset),
    ...
    client.run(
        "install . {} -s compiler.cppstd=14 {} -s arch={}".format(
            " ".join(configs), msvc, arch
        )
    )

    presets = json.loads(client.load("build/14/Release/generators/CMakePresets.json"))

    toolset_value = {"x86_64": "v141,host=x86_64", "x86": "v141,host=x86"}.get(
        arch_toolset
    )
    ...
    arch_value = {"x86_64": "x64", "x86": "Win32"}.get(
        arch
    )  # NOTE: Win32 is different!!
...
def test_user_presets_custom_location(path):
    ...
    if path is not False:
        assert not os.path.exists(
            os.path.join(client.current_folder, "CMakeUserPresets.json")
        )
        assert os.path.exists(
            os.path.join(client.current_folder, "subproject", "CMakeUserPresets.json")
        )
    else:
        assert not os.path.exists(
            os.path.join(client.current_folder, "CMakeUserPresets.json")
        ...
        assert not os.path.exists(
            os.path.join(client.current_folder, "False", "CMakeUserPresets.json")
        )
...
def test_set_cmake_lang_compilers_and_launchers():
    ...
    conanfile = (
        GenConanfile()
        .with_settings("os", "arch", "compiler")
        .with_generator("CMakeToolchain")
    )
    client.save({"conanfile.py": conanfile, "profile": profile})
    client.run("install . -pr:b profile -pr:h profile")
...
def test_cmake_layout_toolchain_folder():
    ...
    c.run(
        "install . -s os=Linux -s compiler=gcc -s compiler.version=7 -s build_type=Release "
        "-s compiler.libcxx=libstdc++11"
    )
    assert os.path.exists(
        os.path.join(c.current_folder, "build/Release/generators/conan_toolchain.cmake")
    )
    c.run(
        "install . -s os=Linux -s compiler=gcc -s compiler.version=7 -s build_type=Debug "
        "-s compiler.libcxx=libstdc++11"
    )
    assert os.path.exists(
        os.path.join(c.current_folder, "build/Debug/generators/conan_toolchain.cmake")
    )
    c.run(
        "install . -s os=Linux -s compiler=gcc -s compiler.version=7 -s build_type=Debug "
    ...
    assert os.path.exists(
        os.path.join(
            c.current_folder, "build/x86-debug/generators/conan_toolchain.cmake"
        )
    )
    c.run(
        "install . -s os=Linux -s compiler=gcc -s compiler.version=7 -s build_type=Debug "
    ...
    assert os.path.exists(
        os.path.join(
            c.current_folder, "build/linux/Debug/generators/conan_toolchain.cmake"
        )
    )
...
def test_build_folder_vars_editables():
    ...
    assert os.path.exists(
        os.path.join(c.current_folder, "dep", "build", "freebsd-debug")
    )
...
def test_test_package_layout():
    ...
    assert os.path.exists(
        os.path.join(client.current_folder, "test_package", build_folder)
    )
    client.run(f"create . {config} -s compiler.cppstd=17")
    build_folder2 = client.created_test_build_folder("pkg/0.1")
    assert os.path.exists(
        os.path.join(client.current_folder, "test_package", build_folder2)
    )
...
def test_presets_not_found_error_msg():
    ...
    assert (
        "Check that you are using CMakeToolchain as generator "
...
def test_recipe_build_folders_vars():
    ...
    client.save({"conanfile.py": conanfile})
    client.run(
        "install . -s os=Windows -s arch=armv8 -s build_type=Debug -o shared=True"
    )
    presets = client.load("build/windows-shared/Debug/generators/CMakePresets.json")
    assert "conan-windows-shared-debug" in presets
    client.run(
        "install . -s os=Linux -s arch=x86 -s build_type=Release -o shared=False"
    )
    ...
    client.run(
        "create . -s os=Windows -s arch=armv8 -s build_type=Debug -o shared=True"
    )
    build_folder = client.created_layout().build()
    presets = load(
        os.path.join(
            build_folder, "build/windows-shared/Debug/generators/CMakePresets.json"
        )
    )
    assert "conan-windows-shared-debug" in presets
...
def test_build_folder_vars_self_name_version():
    ...
    # CLI override has priority
    client.run(
        "install . -s os=Linux  -s build_type=Release "
    ...
    presets = load(
        os.path.join(
            build_folder, "build/windows-pkg-0.1/Debug/generators/CMakePresets.json"
        )
    )
    assert "conan-windows-pkg-0.1-debug" in presets
    ...
    # If we change the conf ``build_folder_vars``, it doesn't affect the cache build
    client.run(
        "create . -s os=Windows -s build_type=Debug "
        "-c tools.cmake.cmake_layout:build_folder_vars='[\"settings.os\"]'"
    )
    build_folder = client.created_layout().build()
    presets = load(
...
def test_build_folder_vars_constants_user():
    ...
    presets = load(
        os.path.join(
            c.current_folder, "build/myvalue/Debug/generators/CMakePresets.json"
        )
    )
...
def test_extra_flags():
    ...
    assert (
        'string(APPEND CONAN_SHARED_LINKER_FLAGS " extra_sharedlinkflags sharedlinkflags")'
        in toolchain
    )
    assert (
        'string(APPEND CONAN_EXE_LINKER_FLAGS " extra_exelinkflags exelinkflags")'
        in toolchain
    )
...
def test_avoid_ovewrite_user_cmakepresets():
    ...
    if shutil.which("cmake") is None:
        pytest.skip("Required 'cmake' not found, skipping test")
    c = TestClient()
    c.save({"conanfile.txt": "", "CMakePresets.json": "{}"})
    c.run("install . -g CMakeToolchain", assert_error=True)
    assert (
        "Error in generator 'CMakeToolchain': Existing CMakePresets.json not generated"
        in c.out
    )
...
def test_add_cmakeexe_to_presets():
    ...
    assert cmake_exe == os.path.basename(
        presets["configurePresets"][0].get("cmakeExecutable")
    )

    # if we set "tools.cmake:cmake_program" that will have preference
    c.run(
        "install . -g CMakeToolchain -g CMakeDeps -c tools.cmake:cmake_program='/other/path/cmake'"
    )
    presets = json.loads(c.load(presets_path))
    ...
    conanfile = textwrap.dedent("""
        ...
        class Pkg(ConanFile):
            ...
            def layout(self):
                cmake_layout(self)
        """)
    c.save({"conanfile.py": conanfile, "CMakeLists.txt": ""})
    c.run("install . ")
...
def test_customize_cmakeuserpresets():
    ...
    c = TestClient()
    c.save({"conanfile.py": GenConanfile(), "CMakeLists.txt": ""})
    ...
    c.run(
        "install . -g CMakeToolchain -of=build -c tools.cmake.cmaketoolchain:user_presets=my.json"
    )
    new = c.load("my.json")
    assert old == new
    # also in subfolder
    c.run(
        "install . -g CMakeToolchain -of=build "
    ...
    c.run(
        "install . -g CMakeToolchain -of=build -c tools.cmake.cmaketoolchain:user_presets="
    )
    assert not os.path.exists(os.path.join(c.current_folder, "CMakeUserPresets.json"))
...
def test_toolchain_extra_variables():
    ...
    client = TestClient()
    client.save(
        {"conanfile.txt": "[generators]\nCMakeToolchain", "windows": windows_profile}
    )
    ...
    toolchain = client.load("conan_toolchain.cmake")
    assert (
        'set(CMAKE_GENERATOR_INSTANCE "${GENERATOR_INSTANCE}/buildTools/")' in toolchain
    )
    ...
    assert (
        'tools.cmake.cmaketoolchain:extra_variables "myVar" "cache" must be a boolean'
        in client.out
    )
    ...
    assert (
        'tools.cmake.cmaketoolchain:extra_variables "myVar" "force" is only allowed for cache variables'
        in client.out
    )
    ...
    assert (
        'tools.cmake.cmaketoolchain:extra_variables "myVar" "force" must be a boolean'
        in client.out
    )
    ...
    assert (
        'tools.cmake.cmaketoolchain:extra_variables "myVar" needs "type" defined for cache variable'
        in client.out
    )
    ...
    assert (
        'tools.cmake.cmaketoolchain:extra_variables "myVar" invalid type "INVALID_TYPE" for cache variable. Possible types: BOOL, FILEPATH, PATH, STRING, INTERNAL'
        in client.out
    )
    ...
    toolchain = client.load("conan_toolchain.cmake")
    assert (
        'set(CACHE_VAR_DEFAULT_DOC "hello world" CACHE PATH "CACHE_VAR_DEFAULT_DOC")'
        in toolchain
    )
...
def test_variables_wrong_scaping():
    ...
    toolchain = c.load("pkg/conan_toolchain.cmake")
    assert (
        'list(PREPEND CMAKE_PROGRAM_PATH "${CMAKE_CURRENT_LIST_DIR}/full_deploy'
        in toolchain
    )
...
def test_thread_flags(threads, flags):
    ...
    if shutil.which("emcc") is None:
        pytest.skip("Required 'emcc' not found, skipping test")
    client = TestClient()
